...
function obtenerDatosRUNT(vehicle, DatosRunt) {
    ...
    for (var dato of datos) {
        ...
        if (dato.includes("PLACA DEL VEHÍCULO")) {
            ...
            vehicle.service_type = lineatipo[1].replace(
                "CLASE DE VEHÍCULO",
                ""
            );
            ...
            vehicle.engine = lineacilindraje[1].replace(
                "TIPO DE CARROCERÍA",
                ""
            );
            ...
            dateControl.value = vehicle.soat_exp; /**Posteriormente se guarda en el input date la fecha de vencimiento tomada por el RUNT */
            break;
        }
    }
    console.log(vehicle)
